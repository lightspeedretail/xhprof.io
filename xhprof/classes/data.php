...
class Data
{
	...
	{
		$this->aggregateRequestData($query, array('datetime_from', 'datetime_to', 'host', 'host_id', 'method_callee_id'));
	...
	private function buildQuery(array $query = NULL, array $whitelist = array())
	{
		if(empty($whitelist))
		{
			$whitelist	= array('datetime_from', 'datetime_to', 'host', 'host_id', 'uri', 'uri_id', 'request_id', 'method_callee_id');
		}
		...
		if(isset($query['method_callee_id']))
		{
			$return['where']		.= ' AND `c2`.`callee_id` = :method_callee_id ';
			$return['extra_join']	.= ' INNER JOIN calls c2 ON c2.request_id = r1.id';
			$return['calls_table']	= 'c2';
		}
	...
	private function aggregateRequestData($query, array $whitelist = array())
	{
		...
		$calls_table = isset($sql_query['calls_table'])?$sql_query['calls_table']:'c1';
